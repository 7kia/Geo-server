...
def getWidthRiver(unit1, unit2, db_file):
    ...
    c = sql.execute("SELECT id, name, sub_type, geometry, country FROM water WHERE Crosses(MakeLine(MakePoint(?,?), MakePoint(?,?)), GeomFromGeoJSON(geometry))", (unit1[1], unit1[0], unit2[1], unit2[0]))
    out = {}
