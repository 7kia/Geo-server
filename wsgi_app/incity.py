...
def application(environ, start_response):
    ...
    # print city
    if city != None:
        response = '{"incity":true, "city_name":"' + city[0] + '", "city_lastname":"' + city[1] + '","city_geometry":' + \
                   city[2] + ',"id":' + str(city[3]) + '}'
...
def getCity(point_lat, point_lng, db_file):
    ...
    cur = conn.cursor()
    sql = "SELECT id, geometry, city_name, city_lastname " \
          "FROM city " \
...
def getPlace(point_lat, point_lng):
    ...
    sql = "SELECT id, place_name, place_type, geometry, lat, lng, country, " \
          "Distance(GeomFromGeoJSON(geometry), MakePoint(" + str(point_lng) + "," + str(point_lat) + "))" \
         " as rast " \
         "from place " \
         "where Distance(" \
         "GeomFromGeoJSON(geometry), MakePoint(" + str(
        point_lng) + "," + str(point_lat) + ")) < " + str(MIN_RAST)
    res = cur.execute(sql)
    places = []
    for rec in res:
        place = {}
