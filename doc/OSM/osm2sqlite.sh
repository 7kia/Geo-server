...
spatialite_osm_net -o osm/$1 -d sqlite/$1.sqlite -T rails --railways -tf temp.conf
ping -n 1 -w 2000 192.168.254.254 > /dev/null
spatialite_network -d sqlite/$1.sqlite -T rails -f node_from -t node_to -g geometry --oneway-tofrom oneway_tofrom --oneway-fromto oneway_fromto -n name -o rails_net_data --overwrite-output
ping -n 1 -w 2000 192.168.254.254 > /dev/null
spatialite sqlite/$1.sqlite "CREATE VIRTUAL TABLE rails_net USING VirtualNetwork('rails_net_data')"

spatialite_osm_net -o osm/$1 -d sqlite/$1.sqlite --roads -T roads -tf road_template.conf
ping -n 1 -w 2000 192.168.254.254 > /dev/null
