...
#загрузка узлов
def load_nodes(cur):
	sql = 'SELECT count(*) FROM rails_nodes'
	res = cur.execute(sql)
	for row in res:
		size = row[0]
	sql = 'select node_id, Y(geometry) as lat, X(geometry) as lng from rails_nodes'
	res = cur.execute(sql)
	nodes = []
	count = 0
	for row in res:
	...
	try:
		...
		while offset < num_rows:
			for node in nodes[offset:offset+BUFFER_SIZE]:
				sql = 'UPDATE rails_nodes SET sector=' + str(node['sector']) + ' WHERE node_id=' + str(node['node_id'])
				cur.execute(sql)
			conn.commit()
...
if not table_exists(cur,'rails_nodes'):
	print 'File "%s" is not rails net database' % db_file
	exit(0)


if need_update:
	add_column(cur,'rails_nodes','sector')
else:
	if column_exists(cur,'rails_nodes','sector'):
...
if '-s' not in map(lambda item: item[0],optlist):
	...
	if answer.lower()[0] == 'y':
		...
		create_index(cur,'rails_nodes', 'sector')
else:
	store_sector(nodes,cur,conn)
	create_index(cur,'rails_nodes', 'sector')
